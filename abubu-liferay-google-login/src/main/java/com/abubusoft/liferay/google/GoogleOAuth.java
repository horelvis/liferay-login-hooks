...
public class GoogleOAuth extends BaseStrutsAction {
	...
	public String execute(HttpServletRequest request,
		...
		if ("login".equals(cmd)) {
			...
			GoogleAuthorizationCodeRequestUrl googleAuthorizationCodeRequestUrl = flow
					.newAuthorizationUrl();
			...
			if (Validator.isNotNull(code)) {
				...
				User user = setGoogleCredentials(session,
						themeDisplay.getCompanyId(), userinfo);
	...
	protected User addUser(HttpSession session, long companyId,
			Userinfoplus userinfo) throws Exception {
	...
	protected Credential exchangeCode(long companyId, String authorizationCode,
			String redirectUri) throws CodeExchangeException {

		try {
			GoogleAuthorizationCodeFlow flow = getFlow(companyId);
			GoogleAuthorizationCodeTokenRequest token = flow
	...
	protected GoogleAuthorizationCodeFlow getFlow(long companyId) throws Exception {
		...
		InputStream is = GoogleOAuth.class
				.getResourceAsStream(_CLIENT_SECRETS_LOCATION);
	...
	protected Userinfoplus getUserInfo(Credential credentials)
		...
		Oauth2.Builder builder = new Oauth2.Builder(new NetHttpTransport(),
	...
	protected User setGoogleCredentials(HttpSession session, long companyId,
			Userinfoplus userinfo) throws Exception {
		...
		if (user != null) {
			if (user.getStatus() == WorkflowConstants.STATUS_INCOMPLETE) {
				session.setAttribute("GOOGLE_INCOMPLETE_USER_ID",
						userinfo.getId());
